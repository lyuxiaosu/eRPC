...
for(( i=0;i<${#throughput_percentage[@]};i++ )) do
        ...
        total_throughput=$(echo "($base_throughput * $threads_count * ${throughput_percentage[i]} / 100)" | bc)
	...
	#ssh -o stricthostkeychecking=no -i ./id_rsa xiaosuGW@$remote_ip "likwid-powermeter sudo $path/start_test.sh $threads_count 3 5 $dispatcher_policy  $server_log $disable_busy_loop $disable_autoscaling > $cpu_log 2>&1 &"
        ...
        ssh -o stricthostkeychecking=no -i ./id_rsa xiaosuGW@$remote_ip "sudo $path/compare_dispatchers.sh 6 1 3 $dispatcher_policy $scheduler $server_log $disable_busy_loop false $disable_preempt $disable_get_req_from_GQ hash.json > $cpu_log 2>&1 &"
