...
class ClientContext : public BasicAppContext {
 public:
  ...
  ~ClientContext() {
	...
	if (log_normal_nums) {
		free(log_normal_nums);
		log_normal_nums = NULL;
	}
  }
};
...
void client_func(erpc::Nexus *nexus, size_t thread_id) {
  ...
  c.log_normal_nums = static_cast<uint32_t*> (malloc(max_requests * sizeof(uint32_t)));
  memset(c.log_normal_nums, 0, max_requests * sizeof(uint32_t));

  double median_x = 0.68386; //e^u
  ...
  while (tmp_counter != max_requests && ctrl_c_pressed != 1) {
	...
	double random_log_normal = distribution(generator);
	...
	if (log_normal_num > max_log_normal_value) {
		max_log_normal_value = log_normal_num;
	} 
	if (log_normal_num < min_log_normal_value) {
		min_log_normal_value = log_normal_num;
	}

	c.log_normal_nums[tmp_counter] = log_normal_num;

	sprintf(reinterpret_cast<char *>(req_msgbuf->buf_), "%u", log_normal_num);
  ...
  for (size_t i = 0; i < max_requests; i++) {
  	fprintf(perf_log, "%zu %d %f %d %u\n", thread_id, req_type_array[thread_id], c.latency_array[i], c.pure_cpu_time[i], c.log_normal_nums[i]);
  }
  close_sessions(c);
}
