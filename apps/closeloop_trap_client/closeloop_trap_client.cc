...
inline void send_req(ClientContext &c, size_t ws_i, size_t req_type) {
  c.start_time.reset();
  c.rpc_->enqueue_request(c.round_robin_get_session_num(), req_type,
...
void app_cont_func2(void *_context, void *_tag) {
  ...
  //printf("%s\n", resp_msgbuf->buf_);
  //printf("%f\n", req_lat_us);
  printf("get response from app_cont_func2 thread id %zu\n", c->thread_id);
  c->num_resps++;
...
void app_cont_func(void *_context, void *_ws_i) {
  ...
  if (c->resp_msgbuf[ws_i].buf_[0] != '0') {
  ...
  if (c->trap_has_sent == 0) {
    uint64_t diff_ts = erpc::rdtsc() - c->start_ts;
    if (diff_ts >= c->total_cycles_when_send_trap) {
      sprintf(reinterpret_cast<char *>(c->req_msgbuf[ws_i].buf_), "%lu", c->trap_type);
      send_req(*c, ws_i, c->trap_type);
      c->trap_has_sent = 1;
      printf("thread id %zu send trap %zu request\n", c->thread_id, c->trap_type);
    } else {
      sprintf(reinterpret_cast<char *>(c->req_msgbuf[ws_i].buf_), "%u", 4);
      send_req(*c, ws_i, 3);
    }
  } else {
    sprintf(reinterpret_cast<char *>(c->req_msgbuf[ws_i].buf_), "%u", 4);
...
void client_func(erpc::Nexus *nexus, size_t trap_type, size_t thread_id) {
  ...
  if (trap_type == 1) {
    printf("trap type is %zu\n", c.trap_type);
    c.total_cycles_when_send_trap = erpc::ms_to_cycles(5000, freq_ghz);	
  } else {
    printf("trap type is %zu\n", c.trap_type);
    c.total_cycles_when_send_trap = erpc::ms_to_cycles(10000, freq_ghz);	
  }
  create_sessions(c);
  ...
  for (size_t i = 0; i < FLAGS_window_size; i++) {
    ...
    sprintf(reinterpret_cast<char *>(c.req_msgbuf[i].buf_), "%u", 4);
    send_req(c, i, 3);
  }
  ...
  while ((end - begin) < total_cycles && ctrl_c_pressed != 1) {
    ...
    throughput_map[thread_id].push_back(c.num_resps / (seconds * Mi(1)));
    c.num_resps = 0;
    c.latency.reset();
    end = erpc::rdtsc();
  }
...
int main(int argc, char **argv) {
  ...
  for (size_t i = 0; i < num_threads; i++) {
    threads[i] = std::thread(client_func, &nexus, i+1, i);
  ...
  fprintf(perf_log, "throughput with timeline:\n");
  printf("throughput with timeline:\n");
